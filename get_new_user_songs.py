...
def get_user_tracks(user_id):
    ...
    """
    new_users_info = pd.DataFrame({"user_id": [], "user_name": [], "playlist_id": [
    ], "playlist_name": [], "track_id": [], "track_name": [], "artist_id": [], "genres": []})
    i = 0
    j = 0

    tracks_info = pd.DataFrame({"track_id": [], "artist_id": [], "genres": [], "danceability": [], "energy": [], "key": [], "loudness": [], "mode": [], "speechiness": [
    ], "acousticness": [], "instrumentalness": [], "liveness": [], "valence": [], "tempo": [], "track_href": [], "duration_ms": [], "time_signature": []})
    ...
    for playlist_id in playlists:
        ...
        for i, track_id in enumerate(track_ids):
            ...
            if len(genres) == 0:
                continue
            new_users_info.loc[i] = [user_id, user_name, playlist_id,
                                     playlist_name, track_id, track_name, artist_ids[i], genres]
            i += 1
            audio_features = sp.audio_features(track_id)[0]
            # print(audio_features.keys())
            if track_id not in list(tracks_info['track_id']):
                tracks_info.loc[j] = [track_id, artist_id, genres, audio_features['danceability'], audio_features['energy'], audio_features["key"], audio_features["loudness"], audio_features["mode"], audio_features["speechiness"],
                                      audio_features["acousticness"], audio_features["instrumentalness"], audio_features["liveness"], audio_features["valence"], audio_features["tempo"], audio_features["track_href"], audio_features["duration_ms"], audio_features["time_signature"]]
                j += 1
    ...
    try:
        ...
        scaling_needed = ["danceability", "energy", "loudness", "speechiness",
                          "acousticness", "instrumentalness",	"liveness", "valence"]
        for feature in scaling_needed:
            tracks_info_scaled[feature] = (tracks_info_scaled[feature] - tracks_info_scaled[feature].min())/(
                tracks_info_scaled[feature].max() - tracks_info_scaled[feature].min())
...
def merge_user_track_data(new_users_info, tracks_info):
    ...
    master.rename(columns={"genres_x": "genres",
                  "artist_id_x": "artist_id"}, inplace=True)
    return master
...
def get_genres_represenentative(master):
    ...
    track_info_median_cols = ['danceability', 'energy', 'loudness', 'speechiness',
                              'acousticness', 'instrumentalness', 'liveness', 'valence', 'tempo', 'duration_ms']
    track_info_mode_cols = ['key', 'mode', 'time_signature']

    a = master_explode.groupby(
        'genres')[track_info_median_cols].agg('median').reset_index()
    b = master_explode.groupby('genres')[track_info_mode_cols].agg(
        lambda x: st.mode(x).mode[0]).reset_index()
    c = master_explode.groupby(
        'genres')[['track_id']].agg('count').reset_index()

    genre_representative_audio_features = a.merge(
        b, how="left", on="genres").merge(c, how="left", on="genres")
    genre_representative_audio_features.rename(
        columns={"track_id": "num_tracks_in_genre"}, inplace=True)
    try:
        genre_representative_audio_features.to_csv(
            "New User's Genre Representative Songs.csv", index=False)
...
#a = main("im7ag12vdykkimqsh6v3pqcca")

# user_id = "31u42bkepbdom6eshufguzacdrzm"

# new_users_info,tracks_info = get_user_tracks(user_id)

# master = merge_user_track_data(new_users_info,tracks_info)


# users = ["31u42bkepbdom6eshufguzacdrzm", "31o2xlpfulupjo4ubr4lqu7amvcu","31isrsytv6r4x6tg3biw3mfaax2i"]

# df = pd.read_csv("Audio_features500_lastfm.csv")
# df.head()
# df.iloc[0,12]
# df.columns
# genres = []
# for i in range(df.shape[0]):
#     track_id = df.iloc[i,12]
#     genres.append(get_genre_for_track(track_id))
# df['genres']=pd.Series(genres)
# df.to_csv("v_Genres_Audio_features500_lastfm.csv")


# df['genres'].apply(lambda x: x.count('celtic'))

#import gower
# from scipy.cluster.hierarchy import linkage, fcluster, dendrogram,fclusterdata

# X = master[['key','mode','time_signature','danceability', 'energy','loudness','speechiness','acousticness','instrumentalness','liveness','valence','tempo','duration_ms']]
# y = master[['genres']]
# gower.gower_matrix(X, cat_features = [True, True,True,False,False,False,False,False,False,False,False,False,False])
# dm = gower.gower_matrix(X)
# Zd = linkage(dm)
# cld = fcluster(Zd, 5, criterion='maxclust')
# np.bincount(cld)
# predicted_clusters = cld
# master['clusters'] = predicted_clusters

# master.head()


# playlist_id = "37i9dQZF1DZ06evO0Oxq8L"
...
# sp.audio_features('spotify:track:0yIUl79oGZKSYeLY1KEOxP')


# set SPOTIPY_REDIRECT_URI='http://localhost:8000'


# client_id = 'f5f3b6a279014ef18d16dd383c998363'
# client_secret = '76eaa154820040ccb616c2f7127a399f'
# client_credentials_manager = SpotifyClientCredentials

# from pprint import pprint
# playlist_dict = {}

# x = sp.playlist_items(playlist_id = '37i9dQZF1DZ06evO0Oxq8L')


# sp.track("3CIyK1V4JEJkg02E4EJnDl?").keys()

# sp.track("3CIyK1V4JEJkg02E4EJnDl?")['popularity']


# sp.track("3CIyK1V4JEJkg02E4EJnDl?").keys()
# track_id = "02dRkCEc8Q5ch4TTcnLxOn"
# mytrack = sp.track(track_id)
# len(mytrack['artists'])

# mytrack['artists'][0]

# track_artist = mytrack['artists'][1]['id']
# sp.artist(track_artist)['genres']
# playlist_id = "1lPD8g84o99wGnFqf2xejQ"
